...
AnimationTimer.prototype = {start: function() {
        ...
        var a = this.stopwatch.getElapsedTime(), b = a / this.duration, c = a * (this.timeWarp(b) / b);
...
ImagePainter.prototype = {
    image: undefined,paint: function(b, a) {
...
SpriteAnimator.prototype = {
    start: function(c, b) {
        var e = c.painter.cells, d = c.painter.cellIndex, a = this;
...
Sprite.prototype = {paint: function(a) {
        a.save();
...
var BUBBLE_LIFETIME = 8000, BUBBLE_EXPANSION_RATE = 8, BUBBLE_FILL_STYLE = "rgb(220,220,220)", BUBBLE_SLOW_EXPANSION_RATE = 4, BUBBLE_X_SPEED_FACTOR = 15, BUBBLE_Y_SPEED_FACTOR = 10, FULLY_OPAQUE = 1, FULLY_TRANSPARENT = 0, TWO_PI = Math.PI * 2;
var SmokeBubble = function(f, e, b, c, a) {
    var d = this;
    this.sprite = new Sprite("smoking hole", {paint: function(h, g) {
...
SmokeBubble.prototype = {update: function(a, b) {
        this.sprite.update(a, b)
    },paint: function(a) {
        this.sprite.paint(a)
    }};
var FireParticle = function(f, e, a) {
    var c = this, d = 200, b = "rgba(255,255,0,0.9)";
    this.left = f;
    this.top = e;
    this.radius = a;
    this.originalRadius = a;
    this.lastFlicker = 0;
    this.sprite = new Sprite("fire particle", {paint: function(i, h) {
            var g = +new Date();
            h.save();
...
FireParticle.prototype = {update: function(a, b) {
        this.sprite.update(a, b)
    },paint: function(a) {
...
SmokingHole.prototype = {pause: function() {
...
window.requestNextAnimationFrame = (function() {
    var c = undefined, g = undefined, f = undefined, e = 0, d = navigator.userAgent, b = 0, a = this;
...
Cycle.prototype = {execute: function(a, c, b) {
...
}, Pulse.prototype = {startDimming: function(a) {
        this.dimTimer.start()
    },dim: function(a) {
...
// game.js
var canvas = document.getElementById("game-canvas"), context = canvas.getContext("2d"), LEFT = 1, RIGHT = 2, BACKGROUND_VELOCITY = 32, BAT_CELLS_HEIGHT = 34, BAT_FAST_FLAP_DURATION = 50, BAT_SLOW_FLAP_DURATION = 100, BEE_CELLS_WIDTH = 50, BEE_CELLS_HEIGHT = 50, BRIGHT = 1, BUTTON_CELLS_HEIGHT = 22, BUTTON_CELLS_WIDTH = 30, BUTTON_PACE_VELOCITY = 50, COIN_CELLS_HEIGHT = 28, COIN_CELLS_WIDTH = 28, CREDITS_DISPLAY_TIME = 10000, CREDITS_OPACITY = 0.9, DEFAULT_TOAST_TIME = 3000, DIM = 0.6, INSTRUCTIONS_DIMMING_DELAY = 50, INSTRUCTIONS_DIM_OPACITY = 0.6, EXPLOSION_CELLS_HEIGHT = 62, EXPLOSION_DURATION = 1000, FPS_SLOW_CHECK_DELAY = 1000, FPS_SPEED_THRESHOLD = 40, GAME_CANVAS_DIM_OPACITY = 0.01, GAME_HEIGHT_IN_METERS = 5, GAME_HEIGHT_IN_PIXELS = 400, GAME_OVER_VELOCITY = 8, GAME_OVER_PAUSE_DURATION = 2000, GRAVITY_FORCE = 9.81, LEVEL_ADVANCE_DELAY = 2000, LEVEL_COMPLETE_CONGRATULATIONS = "Level complete. Nice job!", PAUSE_DELAY = 500, PIXELS_PER_METER = GAME_HEIGHT_IN_PIXELS / GAME_HEIGHT_IN_METERS, PLATFORM_HEIGHT = 8, PLATFORM_STROKE_WIDTH = 2, PLATFORM_STROKE_STYLE = "rgb(0,0,0)", PLATFORM_VELOCITY_MULTIPLIER = 5.5, RUBY_CELLS_HEIGHT = 32, RUBY_CELLS_WIDTH = 32, RUNNER_CELLS_HEIGHT = 60, RUNNER_PAGE_FLIP_INTERVAL = 32, RUNNER_JUMP_DURATION = 1000, RUNNER_JUMP_HEIGHT = 120, SAPPHIRE_CELLS_HEIGHT = 32, SAPPHIRE_CELLS_WIDTH = 32, SILENT = true, SNAIL_BOMB_VELOCITY = 800, SNAIL_BOMB_CELLS_HEIGHT = 20, SNAIL_CELLS_WIDTH = 64, SNAIL_CELLS_HEIGHT = 34, START_GAME_MESSAGE = "<p>Good Luck!</p>", STARTING_BACKGROUND_VELOCITY = 0, STARTING_PLATFORM_OFFSET = 0, STARTING_BACKGROUND_OFFSET = 0, STARTING_RUNNER_LEFT = 50, STARTING_RUNNER_TRACK = 1, STARTING_RUNNER_VELOCITY = 0, STARTING_PAGEFLIP_INTERVAL = -1, TWEET_PREAMBLE = "https://twitter.com/intent/tweet?text=I scored ", TWEET_PROLOGUE = " playing this HTML5 Canvas platformer: http://bit.ly/NDV761 &hashtags=html5", slowFlags = 0, firstSlowFlagTime = 0, lastSlowWarningTime = 0, showSlowWarning = true, runnerHasMoved = false, COIN_VOLUME = 1, SOUNDTRACK_VOLUME = 0.12, JUMP_WHISTLE_VOLUME = 0.05, LANDING_VOLUME = 0.2, FALLING_WHISTLE_VOLUME = 0.1, EXPLOSION_VOLUME = 0.25, SECONDARY_EXPLOSION_VOLUME = 0.25, CHIMES_VOLUME = 1, TRACK_1_BASELINE = 323, TRACK_2_BASELINE = 223, TRACK_3_BASELINE = 123, BAT_SCORE = 20, BEE_SCORE = 30, COIN_SCORE = 25, RUBY_SCORE = 100, SAPPHIRE_SCORE = 250, soundOn = true, audioTracks = [new Audio(), new Audio(), new Audio(), new Audio(), new Audio(), new Audio(), new Audio(), new Audio()], copyright = document.getElementById("copyright"), soundtrack = document.getElementById("soundtrack"), runningSlowly = document.getElementById("running-slowly"), slowlyOkay = document.getElementById("slowly-okay"), slowlyDontShow = document.getElementById("slowly-dont-show"), chimesSound = document.getElementById("chimes-sound"), explosionSound = document.getElementById("explosion-sound"), fallingWhistleSound = document.getElementById("whistle-down-sound"), coinSound = document.getElementById("coin-sound"), jumpWhistleSound = document.getElementById("jump-sound"), landingSound = document.getElementById("thud-sound"), toast = document.getElementById("toast"), soundAndMusic = document.getElementById("sound-and-music"), instructions = document.getElementById("instructions"), scoreElement = document.getElementById("score"), tweetElement = document.getElementById("tweet"), lifeIconLeft = document.getElementById("life-icon-left"), lifeIconMiddle = document.getElementById("life-icon-middle"), lifeIconRight = document.getElementById("life-icon-right"), creditsElement = document.getElementById("credits"), newGameLink = document.getElementById("new-game-link"), soundCheckbox = document.getElementById("sound-checkbox"), musicCheckbox = document.getElementById("music-checkbox"), fpsToast = document.getElementById("fps"), leftControl = document.getElementById("left-control-div"), rightControl = document.getElementById("right-control-div"), runnerPageflipInterval = STARTING_PAGEFLIP_INTERVAL, score = 0, lives = 3, paused = false, pauseStart = 0, totalTimePaused = 0, countingDown = false, transitioning = false, background = new Image(), spritesheet = new Image(), lastAnimationFrameTime = 0, fps = 60, touchTime = 0, backgroundOffset = STARTING_BACKGROUND_OFFSET, platformOffset = STARTING_PLATFORM_OFFSET, bgVelocity = STARTING_BACKGROUND_VELOCITY, platformVelocity, platformData = [{left: 10,width: 230,height: PLATFORM_HEIGHT,fillStyle: "rgb(150,190,255)",opacity: 1,track: 1,pulsate: false,}, {left: 250,width: 100,height: PLATFORM_HEIGHT,fillStyle: "rgb(150,190,255)",opacity: 1,track: 2,pulsate: false,}, {left: 400,width: 125,height: PLATFORM_HEIGHT,fillStyle: "rgb(250,0,0)",opacity: 1,track: 3,pulsate: false}, {left: 633,width: 100,height: PLATFORM_HEIGHT,fillStyle: "rgb(80,140,230)",opacity: 1,track: 1,pulsate: false,}, {left: 810,width: 100,height: PLATFORM_HEIGHT,fillStyle: "rgb(200,200,0)",opacity: 1,track: 2,pulsate: false}, {left: 1025,width: 100,height: PLATFORM_HEIGHT,fillStyle: "rgb(80,140,230)",opacity: 1,track: 2,pulsate: false}, {left: 1200,width: 125,height: PLATFORM_HEIGHT,fillStyle: "aqua",opacity: 1,track: 3,pulsate: false}, {left: 1400,width: 180,height: PLATFORM_HEIGHT,fillStyle: "rgb(80,140,230)",opacity: 1,track: 1,pulsate: false,}, {left: 1625,width: 100,height: PLATFORM_HEIGHT,fillStyle: "rgb(200,200,0)",opacity: 1,track: 2,pulsate: false}, {left: 1800,width: 250,height: PLATFORM_HEIGHT,fillStyle: "rgb(80,140,230)",opacity: 1,track: 1,pulsate: false}, {left: 2000,width: 100,height: PLATFORM_HEIGHT,fillStyle: "rgb(200,200,80)",opacity: 1,track: 2,pulsate: false}, {left: 2100,width: 100,height: PLATFORM_HEIGHT,fillStyle: "aqua",opacity: 1,track: 3,}, {left: 2269,width: 200,height: PLATFORM_HEIGHT,fillStyle: "gold",opacity: 1,track: 1,}, {left: 2500,width: 200,height: PLATFORM_HEIGHT,fillStyle: "#2b950a",opacity: 1,track: 2,snail: true}, ], batData = [{left: 1150,top: TRACK_2_BASELINE - BAT_CELLS_HEIGHT}, {left: 1720,top: TRACK_2_BASELINE - 2 * BAT_CELLS_HEIGHT}, {left: 2000,top: TRACK_3_BASELINE}, {left: 2200,top: TRACK_3_BASELINE - BAT_CELLS_HEIGHT}, {left: 2400,top: TRACK_3_BASELINE - 2 * BAT_CELLS_HEIGHT}, ], beeData = [{left: 500,top: 64}, {left: 944,top: TRACK_2_BASELINE - BEE_CELLS_HEIGHT - 30}, {left: 1600,top: 125}, {left: 2225,top: 125}, {left: 2295,top: 275}, {left: 2450,top: 275}, ], buttonData = [{platformIndex: 7}, {platformIndex: 12}, ], coinData = [{left: 303,top: TRACK_2_BASELINE - COIN_CELLS_HEIGHT}, {left: 469,top: TRACK_3_BASELINE - 2 * COIN_CELLS_HEIGHT}, {left: 600,top: TRACK_1_BASELINE - COIN_CELLS_HEIGHT}, {left: 833,top: TRACK_2_BASELINE - 2 * COIN_CELLS_HEIGHT}, {left: 1050,top: TRACK_2_BASELINE - 2 * COIN_CELLS_HEIGHT}, {left: 1500,top: TRACK_1_BASELINE - 2 * COIN_CELLS_HEIGHT}, {left: 1670,top: TRACK_2_BASELINE - 2 * COIN_CELLS_HEIGHT}, {left: 1870,top: TRACK_1_BASELINE - 2 * COIN_CELLS_HEIGHT}, {left: 1930,top: TRACK_1_BASELINE - 2 * COIN_CELLS_HEIGHT}, {left: 2200,top: TRACK_3_BASELINE - 3 * COIN_CELLS_HEIGHT}, ], rubyData = [{left: 200,top: TRACK_1_BASELINE - RUBY_CELLS_HEIGHT}, {left: 880,top: TRACK_2_BASELINE - RUBY_CELLS_HEIGHT}, {left: 1100,top: TRACK_2_BASELINE - 2 * SAPPHIRE_CELLS_HEIGHT}, {left: 1475,top: TRACK_1_BASELINE - RUBY_CELLS_HEIGHT}, ], sapphireData = [{left: 680,top: TRACK_1_BASELINE - SAPPHIRE_CELLS_HEIGHT}, {left: 1700,top: TRACK_2_BASELINE - SAPPHIRE_CELLS_HEIGHT}, {left: 2056,top: TRACK_2_BASELINE - 3 * SAPPHIRE_CELLS_HEIGHT / 2}, {left: 2333,top: TRACK_2_BASELINE - SAPPHIRE_CELLS_HEIGHT}, ], smokingHoleData = [{left: 248,top: TRACK_2_BASELINE - 22}, {left: 850,top: TRACK_1_BASELINE - 122}, ], snailData = [{platformIndex: 13}, ], snailBombData = [{left: 2585,top: TRACK_2_BASELINE - SNAIL_CELLS_HEIGHT + SNAIL_BOMB_CELLS_HEIGHT / 2}], batCells = [{left: 1,top: 0,width: 32,height: BAT_CELLS_HEIGHT}, {left: 38,top: 0,width: 46,height: BAT_CELLS_HEIGHT}, {left: 90,top: 0,width: 32,height: BAT_CELLS_HEIGHT}, {left: 129,top: 0,width: 46,height: BAT_CELLS_HEIGHT}, ], batRedEyeCells = [{left: 185,top: 0,width: 32,height: BAT_CELLS_HEIGHT}, {left: 222,top: 0,width: 46,height: BAT_CELLS_HEIGHT}, {left: 273,top: 0,width: 32,height: BAT_CELLS_HEIGHT}, {left: 313,top: 0,width: 46,height: BAT_CELLS_HEIGHT}, ], beeCells = [{left: 5,top: 234,width: BEE_CELLS_WIDTH,height: BEE_CELLS_HEIGHT}, {left: 75,top: 234,width: BEE_CELLS_WIDTH,height: BEE_CELLS_HEIGHT}, {left: 145,top: 234,width: BEE_CELLS_WIDTH,height: BEE_CELLS_HEIGHT}], buttonCells = [{left: 2,top: 190,width: BUTTON_CELLS_WIDTH,height: BUTTON_CELLS_HEIGHT}, {left: 45,top: 190,width: BUTTON_CELLS_WIDTH,height: BUTTON_CELLS_HEIGHT}], coinCells = [{left: 2,top: 540,width: COIN_CELLS_WIDTH,height: COIN_CELLS_HEIGHT}], explosionCells = [{left: 1,top: 48,width: 50,height: EXPLOSION_CELLS_HEIGHT}, {left: 60,top: 48,width: 68,height: EXPLOSION_CELLS_HEIGHT}, {left: 143,top: 48,width: 68,height: EXPLOSION_CELLS_HEIGHT}, {left: 230,top: 48,width: 68,height: EXPLOSION_CELLS_HEIGHT}, {left: 305,top: 48,width: 68,height: EXPLOSION_CELLS_HEIGHT}, {left: 389,top: 48,width: 68,height: EXPLOSION_CELLS_HEIGHT}, {left: 470,top: 48,width: 68,height: EXPLOSION_CELLS_HEIGHT}], goldButtonCells = [{left: 88,top: 190,width: BUTTON_CELLS_WIDTH,height: BUTTON_CELLS_HEIGHT}, {left: 130,top: 190,width: BUTTON_CELLS_WIDTH,height: BUTTON_CELLS_HEIGHT}], rubyCells = [{left: 3,top: 135,width: RUBY_CELLS_WIDTH,height: RUBY_CELLS_HEIGHT}, {left: 39,top: 135,width: RUBY_CELLS_WIDTH,height: RUBY_CELLS_HEIGHT}, {left: 76,top: 135,width: RUBY_CELLS_WIDTH,height: RUBY_CELLS_HEIGHT}, {left: 112,top: 135,width: RUBY_CELLS_WIDTH,height: RUBY_CELLS_HEIGHT}, {left: 148,top: 135,width: RUBY_CELLS_WIDTH,height: RUBY_CELLS_HEIGHT}], runnerCellsRight = [{left: 414,top: 375,width: 47,height: RUNNER_CELLS_HEIGHT}, {left: 362,top: 375,width: 44,height: RUNNER_CELLS_HEIGHT}, {left: 314,top: 375,width: 39,height: RUNNER_CELLS_HEIGHT}, {left: 265,top: 375,width: 46,height: RUNNER_CELLS_HEIGHT}, {left: 205,top: 375,width: 49,height: RUNNER_CELLS_HEIGHT}, {left: 150,top: 375,width: 46,height: RUNNER_CELLS_HEIGHT}, {left: 96,top: 375,width: 42,height: RUNNER_CELLS_HEIGHT}, {left: 45,top: 375,width: 35,height: RUNNER_CELLS_HEIGHT}, {left: 0,top: 375,width: 35,height: RUNNER_CELLS_HEIGHT}], runnerCellsLeft = [{left: 0,top: 295,width: 47,height: RUNNER_CELLS_HEIGHT}, {left: 55,top: 295,width: 44,height: RUNNER_CELLS_HEIGHT}, {left: 107,top: 295,width: 39,height: RUNNER_CELLS_HEIGHT}, {left: 152,top: 295,width: 46,height: RUNNER_CELLS_HEIGHT}, {left: 208,top: 295,width: 49,height: RUNNER_CELLS_HEIGHT}, {left: 265,top: 295,width: 46,height: RUNNER_CELLS_HEIGHT}, {left: 320,top: 295,width: 42,height: RUNNER_CELLS_HEIGHT}, {left: 380,top: 295,width: 35,height: RUNNER_CELLS_HEIGHT}, {left: 425,top: 295,width: 35,height: RUNNER_CELLS_HEIGHT}, ], sapphireCells = [{left: 185,top: 135,width: SAPPHIRE_CELLS_WIDTH,height: SAPPHIRE_CELLS_HEIGHT}, {left: 220,top: 135,width: SAPPHIRE_CELLS_WIDTH,height: SAPPHIRE_CELLS_HEIGHT}, {left: 258,top: 135,width: SAPPHIRE_CELLS_WIDTH,height: SAPPHIRE_CELLS_HEIGHT}, {left: 294,top: 135,width: SAPPHIRE_CELLS_WIDTH,height: SAPPHIRE_CELLS_HEIGHT}, {left: 331,top: 135,width: SAPPHIRE_CELLS_WIDTH,height: SAPPHIRE_CELLS_HEIGHT}], snailBombCells = [{left: 2,top: 512,width: 30,height: 20}], snailCells = [{left: 142,top: 466,width: SNAIL_CELLS_WIDTH,height: SNAIL_CELLS_HEIGHT}, {left: 75,top: 466,width: SNAIL_CELLS_WIDTH,height: SNAIL_CELLS_HEIGHT}, {left: 2,top: 466,width: SNAIL_CELLS_WIDTH,height: SNAIL_CELLS_HEIGHT}, ], explosionAnimator = new SpriteAnimator(explosionCells, EXPLOSION_DURATION, function(b, a) {
    b.exploding = false
}), gameTimer = new AnimationTimer(1, AnimationTimer.makeLinear(1)), runInPlace = {lastAdvanceTime: 0,execute: function(a, c, b) {
    ...
    }}, moveLaterally = {lastMove: 0,execute: function(b, d, c) {
        ...
        var c = d.jumpAscendTimer.getElapsedTime(), b = c / (d.JUMP_DURATION / 2), a = b * d.JUMP_HEIGHT_IN_PIXELS;
        ...
        var f = e.left + e.width, c = e.left + e.width / 2, b = e.left, g = d.left - d.offset, a = g + d.width;
    ...
    }}, runnerCollide = {didCollide: function(b, g) {
        var f = b.left + b.width / 2, d = b.top + b.height / 2, a = b.top + b.height, e = g.left - g.offset, c = false;
        if (f > e && f < e + g.width) {
    ...
    }}, pace = {execute: function(b, e, d) {
        var c = b.left + b.width, f = b.platform.left + b.platform.width, a = b.velocityX / d;
    ...
    }}, snailBombShoot = {execute: function(a, d, c) {
        var b = a.bomb;
        if (!spriteInView(a)) {
            return
        }
    ...
    }}, snailBombCollide = {execute: function(h, d, c) {
        var i = h.left - h.offset + h.width, f = h.left - h.offset + h.width / 2, g = h.left, b = runner.left, e = b + runner.width, a = false;
    ...
    }}, snailBombMove = {execute: function(a, c, b) {
    ...
    }}, runnerPainter = new SpriteSheetPainter(spritesheet, runnerCellsRight), platformPainter = {paint: function(b, a) {
        var c;
        a.save();
        ...
        a.restore()
    }}, bats = [], bees = [], buttons = [], coins = [], platforms = [], rubies = [], sapphires = [], smokingHoles = [], snails = [], snailBombs = [], runner = new Sprite("runner", runnerPainter), sprites = [runner];
function draw(a) {
    setPlatformVelocity();
